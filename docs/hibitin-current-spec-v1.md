...
- 今日のひとおしは通常ルーティンとは独立し、達成率、PT、熟練度、スタンプ帳には影響しません。
...
- 日替わりの問い: `dailyOneLinePrompts` から `daily-one-line:YYYY-MM-DD` の安定ハッシュで選択
- 問いの保存: なし。回答のみ既存キーへ保存
- プレースホルダー: `ひとこと書いてみよう`
- 自動保存
...
- 今日タブで、起床セクションより上に独立カードとして表示します。
- 昨日/今日切り替えに連動し、選択中の日付のひとおしを表示します。
- 有効候補が0件の場合は `今日のひとおしは準備中です` と表示します。
...
- 達成率、PT、星、トロフィー、ランク、ショップ、スタンプ帳には影響しません。

管理:

- 開発者モードの設定画面に `今日のひとおし管理` を表示します。
...
| 今日のひとおし管理 | `hibitin:dailyNudgeCandidates:v1` | 今後の日付抽選に連動 | 候補20件 |
...
`今日のひとおし管理` はゲームバランス設定とは別の開発者向け設定ブロックです。既に割り当て済みの日付記録は `hibitin:dailyNudgeRecords:v1` 側に文章ごと保存されるため、候補編集・削除の影響を受けません。
