...
- なんでもメモは分類しない仮置き用の自由メモで、文字列として日付別に保存します。
...
- 保存キー: `hibitin:todos:v2`
- 日付切り替え実行済みキー: `hibitin:todos:lastRolloverDate:v1`
...
- 保存キー: `hibitin:anyMemo:YYYY-MM-DD`
- データ構造: `string`
- UI: Today画面の最下部、スタンプ帳詳細、記録タブの日付カードの保存済み表示/編集パネル内に「なんでもメモ」として表示
- プレースホルダー: `とりあえず、ここにメモ`
- 1つのtextareaで、文章量や改行に応じて自動で縦へ伸びる
- 自動保存で、Today画面、スタンプ帳詳細、記録タブは同じ保存データを読み書きする
- 分類しない仮置き用の便利メモで、PT、達成率、FIRST/PERFECT判定、スタンプ、Rank、星、トロフィー、コアルーティン、フリークエスト、今日のやること件数には影響しない
- バックアップ、復元、初期状態リセットは `hibitin:` 動的キーとして対象
...
- 初期配置は `今日のひとことを残す` が昼、`今日のできごとを残す` が夜です。
...
- 昨日/今日切り替えに連動し、選択中の日付の日替わりクエストを表示します。
- 有効候補が0件の場合は `日替わりクエストは準備中です` と表示します。
...
- 開発者モードの設定画面に `日替わりクエスト管理` を表示します。
